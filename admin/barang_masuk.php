            ...
            <div class="table-responsive">
              <table class="table table-bordered table-striped" id="table-datatable">
                ...
                <tbody>
                  ...
                  include '../koneksi.php';
                  $no=1;
                  $query = "SELECT bm.bm_id, b.barang_nama, bm.bm_tgl_masuk, bm.bm_jumlah, s.suplier_nama 
                            FROM barang_masuk bm 
                            INNER JOIN barang b ON bm.bm_id_barang = b.barang_id 
                            INNER JOIN suplier s ON bm.bm_id_suplier = s.suplier_id";
                  $data = mysqli_query($koneksi, $query);
                  while($d_entry = mysqli_fetch_array($data)){
                    ?>
                    <tr>
                      <td><?php echo $no++; ?></td>
                      <td><?php echo $d_entry['barang_nama']; ?></td>
                      <td><?php echo $d_entry['bm_tgl_masuk']; ?></td>
                      <td><?php echo $d_entry['bm_jumlah']; ?></td>
                      <td><?php echo $d_entry['suplier_nama']; ?></td>
                      <td>                        
                        <a class="btn btn-danger btn-sm" href="barang_masuk_hapus.php?id=<?php echo $d_entry['bm_id'] ?>"><i class="fa fa-trash"></i></a>
                      </td>
                    </tr>
                    <?php 
                  }
